...
void CartoonizeOMP::process(const Parameters &params, const Image &src, Image &dst)
{
   ...
   printf("\n *********** FINISHED *********** \n");
   printf("ALGORITHM: CARTOONIZE with OpenMP\n");
   cout << "RUNTIME: " << elapsed.count() << " ms" << endl;
   cout << endl;
}

double** CartoonizeOMP::createMatrix(int height, int width) {
...
double CartoonizeOMP::euklid(int r, int g, int b) {
   return sqrt(pow(r, 2) + pow(g, 2) + pow(b, 2));
}

double CartoonizeOMP::calcDomain(int y, int x, int k, int l) {
...
double CartoonizeOMP::calcEdge(const Image &img, int y, int x, int k, int l) {
   double r, g, b;
...
double CartoonizeOMP::calcKernelField(const Image &img, int y, int x, int k, int l) {
   return calcDomain(y, x, k, l) * calcEdge(img, y, x, k, l);
}

bool CartoonizeOMP::isInRange(int size, int y, int x, const int height, const int width) {
...
Pixel CartoonizeOMP::bilKernel(const Image &img, int y, int x) {
...
Image CartoonizeOMP::bilFilter(const Image &img) {
...
Pixel CartoonizeOMP::calcEdgeStrength(int y, int x, Image &imgX, Image &imgY) {
   double r, g, b;
...
Pixel CartoonizeOMP::calcEdgeDetection(int y, int x, Image &img) {
   double r, g, b;
...
Image CartoonizeOMP::edgeStrength(Image &imgX, Image &imgY) {
...
Image CartoonizeOMP::edgeDetection(Image &img) {
   const int height = img.height();
   const int width = img.width();
   Image result = Image(height, width);
...
double** CartoonizeOMP::getSobelKernel(char c) {
...
double** CartoonizeOMP::getBlurKernel() {
   double** kernel = createMatrix(3, 3);
...
double CartoonizeOMP::adjustPixelCol(double pixel) {
   if (pixel > 255.0)
      return 255.0;
...
Pixel CartoonizeOMP::binKernel(const Image &img, int y, int x, char op) {
   int height = 0;
   int width  = 0;
   double r = 0.0;
   double g = 0.0;
...
Image CartoonizeOMP::binFilter(const Image &img, char op) {
   const int height = img.height();
   const int width = img.width();
   Image result = Image(height, width);
...
Image CartoonizeOMP::binTraverse(const Image &img) {
//   Image g;
   Image gX;
   Image gY;
   Image G;
...
Pixel CartoonizeOMP::comp(const Pixel &pixBilateral, const Pixel &pixBinary) {
   double r, g, b;
   r = pixBinary.r;
   g = pixBinary.g;
   b = pixBinary.b;
...
Image CartoonizeOMP::cartFilter(Image &bilateral, Image &binary) {
